  ...
  const geminiCall = async (prompt, systemOverride) => {
    ...
    try {
      const res = await fetch(`/api/ai`, {
        ...
        body: JSON.stringify({ prompt, system: systemText }),
      });
      ...
      // If quota or rate limit, fall back to Ollama proxy
      if (res.status === 429 || (err && err.error && err.error.toLowerCase().includes('quota'))) {
        ...
        if (!r2.ok) {
          ...
          throw new Error(e2?.error || `Ollama error ${r2.status}`);
        }
        const dd = await r2.json();
        return dd.result || "No response";
      }

      throw new Error(err.error || `Server error ${res.status}`);
    } catch (e) {
      // On network/other errors, try Ollama before falling back to a
      // free local heuristic fallback so the UI remains functional.
      try {
        const r2 = await fetch(`/api/ollama`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (!r2.ok) {
          ...
          // continue to local fallback instead of throwing
        } else {
          const dd = await r2.json();
          return dd.result || "No response";
        }
      ...
      // Local no-cost fallback: basic heuristic responder that doesn't call external APIs.
      const localAIFallback = (txt) => {
        const t = (txt || "").toLowerCase();
        if (t.includes("growth") || t.includes("30-day") || t.includes("30 day")) {
          return `Local AI (fallback): 30-day growth plan\n1) Run a 4-week referral drive with a small reward.\n2) Offer limited-time 1-month trial at a promo price.\n3) Run social reels highlighting member wins twice weekly.\n4) Upsell PT packages to top 10% of attendees.\n5) Track signups and follow up via WhatsApp.`;
        }
        if (t.includes("revenue") || t.includes("pricing")) {
          return `Local AI (fallback): Quick revenue ideas\n- Add a low-cost trial to convert fence-sitters.\n- Create a small group training upsell.\n- Introduce a referral bonus for 1 month.\n- Run a short offer and track conversion.`;
        }
        if (t.includes("retention") || t.includes("churn") || t.includes("leave")) {
          return `Local AI (fallback): Retention tips\n1) Reach out to at-risk members with a personalised message.\n2) Offer a 7-day comeback incentive.\n3) Run small competitions and celebrate winners.\n4) Ensure trainers check in with new joiners.`;
        }
        if (t.includes("instagram") || t.includes("caption") || t.includes("reel")) {
          return `Local AI (fallback): Social ideas\n- Caption: \"New month, new goals. Join our 30-day challenge! #gym #fitness\"\n- Reel hook: Quick before/after clip + CTA.\n- CTA: DM us to book a free trial.`;
        }
        // Generic echo with suggestions
        return `Local AI (fallback): I couldn't reach the cloud model right now. Here are some suggestions based on your prompt:\n\nPrompt: ${prompt}\n\nSuggestions:\n1) Be specific about the goal you want (growth/retention/revenue).\n2) Ask for a numbered plan or template.\n3) Ask for WhatsApp message templates if you want copy to send.`;
      };
