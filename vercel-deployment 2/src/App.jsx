...
export default function App() {
  ...
  const [newJoined, setNewJoined] = useState(""); // editable join date (set on client only)
  const [editData, setEditData] = useState(null);

  // ── Initialize client-side state after hydration ───────────────────────────
  useEffect(() => {
    if (newJoined === "") {
      setNewJoined(mkDate(0));
    }
  }, []);
  ...
  // ── GROQ CLOUD API (via server proxy) ─────────────────────────────────────
  ...
  // ── Core AI caller — uses /api/ai proxy (Groq key stays server-side) ───────
